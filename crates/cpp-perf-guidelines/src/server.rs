...
impl CppPerfGuidelinesServer {
    #[tool(
        description = "Search the low-level C++ performance guidelines (custom allocators, data layout, cache behavior, copy/move discipline, object lifetime, embedded constraints, concurrency memory effects, codegen, SIMD and vectorization, telemetry and observability harnesses) by semantic similarity. Returns ranked results."
    )]
    ...
    #[tool(
        description = "List all C++ performance guidelines in a category. Categories: 'memory', 'copy-move', 'cache-layout', 'lifetime', 'embedded', 'concurrency', 'codegen', 'simd', 'telemetry'."
    )]
        ...
        ServerInfo {
            ...
            instructions: Some(
                "Low-level C++ performance guidelines MCP server. Curated technique-level \
                 ...
                 effects, codegen, SIMD/vectorization, and telemetry/observability \
                 harnesses — the layer below the ISO C++ Core Guidelines. Use \
